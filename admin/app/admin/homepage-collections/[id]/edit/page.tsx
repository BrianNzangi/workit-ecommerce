...
export default function EditHomepageCollectionPage() {
    ...
    const fetchCollection = async () => {
        try {
            const response = await fetch(`/api/admin/homepage-collections/${collectionId}`);

            if (!response.ok) {
                throw new Error(response.status === 404 ? 'Collection not found' : 'Failed to load collection');
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast({
                    title: 'Update failed',
                    description: data.error || 'Failed to update homepage collection',
                    variant: 'error',
                });
            }
        } catch (error) {
