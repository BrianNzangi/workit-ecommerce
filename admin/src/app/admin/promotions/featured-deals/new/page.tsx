...
export default function NewFeaturedDealPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch("/api/promotions/featured-deals/admin", {
                method: "POST",
                headers,
                body: JSON.stringify({
                    title: form.title,
            ...
            const data = await res.json();
            if (data.success) {
                toast({ title: "Created", description: "Featured deal created successfully", variant: "success" });
                router.push("/admin/promotions/featured-deals");
            } else {
                toast({ title: "Error", description: data.message || "Failed to create featured deal", variant: "error" });
            }
        } catch {
            toast({ title: "Error", description: "Failed to create featured deal", variant: "error" });
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6 bg-white rounded-lg p-3 sm:p-4">
                        <div className="flex items-center gap-3">
                            <h1 className="text-lg font-semibold">Create New Featured Deal</h1>
                        </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <Card className="rounded-sm shadow-none mb-6">
                            <CardContent className="p-6">
                                <h2 className="text-sm font-semibold mb-4">Add Product for Featured Deal</h2>
