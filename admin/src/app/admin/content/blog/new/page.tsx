...
export default function NewBlogPage() {
    ...
    const handleSave = async (publish = false) => {
        ...
        try {
            ...
            const requestBody: any = { title: formData.title, slug, content, published: publish };
            ...
            if (publish) requestBody.publishedAt = new Date().toISOString();
            const service = new BlogService();
            await service.createBlog(requestBody);
            toast({ title: "Success", description: `Blog post ${publish ? "published" : "saved as draft"} successfully`, variant: "success" });
            router.push("/admin/content/blog");
        } catch (error: any) {
            toast({ title: "Save failed", description: error.message || "Failed to create blog post", variant: "error" });
        } finally {
            setLoading(false);
        }
    };
