...
export default function BannersPage() {
    ...
    const toggleStatus = async (id: string, currentStatus: boolean) => {
        try {
            const bannerService = new BannerService();
            const updatedBanner = await bannerService.updateBanner(id, { enabled: !currentStatus });
        ...
        >
            <td className="px-6 py-4">
                <div className="h-12 w-20 bg-gray-100 rounded overflow-hidden flex items-center justify-center">
                    {banner.desktopImage ? (
                        <img
                            src={getImageUrl(banner.desktopImage?.preview || (banner as any).desktopImage?.source || '')}
                            alt={banner.name}
