...
export async function proxyRequest(request: NextRequest, customEndpoint?: string) {
    ...
    if (customEndpoint) {
        ...
        console.log(`[Proxy Debug] CUSTOM: backendUrl=${backendUrl}, customEndpoint=${customEndpoint}, finalUrl=${url}`);
    } else {
        const { pathname, search } = new URL(request.url);
        // Map /api/admin/products -> /products
        const backendPath = pathname.replace(/^\/api\/admin/, '');
        url = `${backendUrl}${backendPath}${search}`;
        console.log(`[Proxy Debug] MAP: pathname=${pathname}, backendPath=${backendPath}, finalUrl=${url}`);
    }
