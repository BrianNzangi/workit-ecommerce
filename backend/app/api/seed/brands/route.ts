...
export async function POST(request: NextRequest) {
    try {
        ...
        for (const brandData of brands) {
            try {
                ...
                if (existing) {
                    ...
                    results.push({ brand: brandData.name, status: 'updated' });
                    updated++;
                } else {
                    await prisma.brand.create({
                        data: brandData
                    });
                    results.push({ brand: brandData.name, status: 'created' });
                    created++;
                }
            } catch (error: any) {
                results.push({ brand: brandData.name, status: 'error', error: error.message });
                errors++;
            }
        }
        ...
        return NextResponse.json(
            { error: 'Failed to seed brands', details: error.message },
            { status: 500 }
        );
    }
}
