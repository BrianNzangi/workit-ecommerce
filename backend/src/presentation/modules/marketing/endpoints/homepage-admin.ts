...
import { db, schema, eq, asc } from '@workit/db';
...
export const homepageAdminRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  const withProducts = {
    products: {
      ...
      with: {
        product: {
          columns: {
            id: true, name: true, slug: true, salePrice: true, originalPrice: true,
            stockOnHand: true, condition: true, createdAt: true, updatedAt: true, sku: true,
          },
          with: {
            assets: {
              columns: { id: true, productId: true, assetId: true, sortOrder: true, featured: true },
              with: {
                asset: {
  ...
  fastify.get('/', { preHandler: preAdmin }, async () => {
    const collections = await db.query.homepageCollections.findMany({
  ...
  fastify.get('/:id', { preHandler: preAdmin }, async (request, reply) => {
    ...
    if (!collection) return reply.status(404).send({ message: 'Homepage collection not found' });
    return { collection };
  });
  ...
  fastify.post('/', { preHandler: preAdmin }, async (request, reply) => {
    try {
      ...
      if (error.cause?.code === '23505' || error.message?.includes('HomepageCollection_slug_unique')) {
        return reply.status(400).send({ success: false, message: 'A homepage collection with this slug already exists.' });
      }
      throw error;
    }
  });
  ...
  fastify.put('/:id', { preHandler: preAdmin }, async (request, reply) => {
    try {
      ...
      if (!existing) return reply.status(404).send({ message: 'Homepage collection not found' });
