...
import { db, schema, eq, desc, asc, ilike, and } from "../../../lib/db.js";
...
export const blogAdminRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/", {
        ...
        if (published !== undefined) {
            conditions.push(eq(schema.blogs.published, published === 'true'));
        }
        if (search) {
            conditions.push(ilike(schema.blogs.title, `%${search}%`));
        }
