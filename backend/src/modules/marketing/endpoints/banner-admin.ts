...
import { db, schema, eq, desc, asc, ilike, and, inArray } from "../../../lib/db.js";
...
export const bannerAdminRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/", {
        ...
        const conditions = [];
        if (position) {
            conditions.push(eq(schema.banners.position, position));
        }
        if (enabled !== undefined) {
            conditions.push(eq(schema.banners.enabled, enabled === 'true'));
        }
        if (search) {
            conditions.push(ilike(schema.banners.title, `%${search}%`));
        }
    ...
    fastify.get("/:id", {
        ...
        if (!banner) {
            return reply.status(404).send({ message: "Banner not found" });
        }
