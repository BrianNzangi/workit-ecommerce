...
import client from '../../lib/woocommerce';
...
export default function TopDeals() {
  ...
  useEffect(() => {
    const query = `
      *[_type == "collection" && title == "Top Deals"][0] {
        "products": products[]->{
          _id,
          name,
          price,
    ...
    `;
    (client as any).fetch(query).then((data: Product[]) => setProducts(data || []));
  }, []);
        ...
        <div className="flex overflow-hidden">
          ...
          >
            {visibleProducts.map((product) => (
              <div key={product._id} className="min-w-[200px] max-w-[240px] flex-shrink-0">
